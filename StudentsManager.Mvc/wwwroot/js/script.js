            ...
            function On(n) {
                ...
                io = function () {
                    ...
                    yo = zt; (Pi && "[object DataView]" != yo(new Pi(new ArrayBuffer(1))) || Zi && "[object Map]" != yo(new Zi()) || qi && "[object Promise]" != yo(qi.resolve()) || Vi && "[object Set]" != yo(new Vi()) || Ki && "[object WeakMap]" != yo(new Ki())) && (yo = function (n) {
                        var t = zt(n); if (n = (n = "[object Object]" == t ? n.constructor : F) ? Fe(n) : "") switch (n) {
                            case Yi:
            ...
            l = function () {
                function t() {
                    ...
                    key: "load", value: function (t, e, s) {
                        ...
                        } t.strPos = 0, t.arrayPos = 0, t.stopNum = 0, t.loop = t.options.loop, t.loopCount = t.options.loopCount, t.curLoop = 0, t.shuffle = t.options.shuffle, t.sequence = [], t.pause = { status: !1, typewrite: !0, curString: "", curStrPos: 0 }, t.typingComplete = !1; for (var o in t.strings) t.sequence[o] = o; t.currentElContent = this.getCurrentElContent(t), t.autoInsertCss = t.options.autoInsertCss, this.appendAnimationCss(t);
                    }
                }, {
...
var cities = "Варна, София, Бургас, Пловдив, Айтос, Албена, Асеновград, Балчик, Банско, Бенковски, Благоевград, Ботевград, Бургас, Велико Търново, Велинград, Враца, Гoце Делчев, Горна Оряховица, Габрово, Генерал Тошево, Гълъбово, Девин, Димитровград, Добрич, Доспат, Дулово, Дупница, Дългопол, Златоград, Исперих, Каварна, Казанлък, Карлово, Костинброд, Кубрат, Кърджали, Кюстендил, Ловече, Луковит, Монтана, Нова Загора, Пазарджик, Перник, Петрич, Пещера, Плевен, Попово, Провадия, Първомай, Раднево, Разград, Русе, Самоков, Сандански, Свиленград, Свищов, Своге, Севлиево, Силистра, Сливен, Смолян, Стамболойски, Стара Загора, Троян, Търговище, Харманли, Хасково, Червен бряг, Чирпан, Шумен, Ямбол, Тенево, Несебър, Боровец, Златни Пясъци, Слънчев бряг, друг".split(',').map(_.trim);

function getMobileDropdown() {
    var options = cities.map(function (city) {
        return "<option value='" + String(city) + "'>" + String(city) + "</option>";
    });
    return $("<select id=\"city-select\" name=\"city\">" + String(options.join('')) + "</select>");
}

function getDesktopDropdown() {
    var listItems = cities.map(function (city) {
        return "\n            <li>\n                <label class=\"list-value\">\n                    <span>" + String(city) + "</span>\n                    <input type=\"radio\" name=\"city\" value=\"" + String(city) + "\" class=\"hidden\"/>\n                </label>\n            </li>\n        ";
    }).join('');

    return $("<ul class=\"soge-list\">" + String(listItems) + "</ul>");
}

var defaultScaleViewProps = {
    onClick: function () { },
    container: '#scale-container'
};

var ScaleView = function (options) {
    this.options = $(Object).extend(defaultScaleViewProps, options);
    this.bind();
};

ScaleView.prototype = {
    bind: function () {
        $(this.options.container).find('.soge-scale-btn').hover(this.hoverHandler.bind(this)).on('click', this.clickHandler.bind(this)).end().find('.soge-scale').hover($.noop, this.hoverOutHandler.bind(this));
    },
    unbind: function () {
        $(this.options.container).find('.soge-scale-btn').unbind('hover').unbind('click');
    },
    hoverHandler: function (event) {
        var index = $(event.currentTarget).index() + 1;
        var $container = $(this.options.container);
        var $btns = $container.find('.soge-scale-btn');

        clearTimeout(this.hoverHandlerTimeout);

        $btns.removeClass('hover').slice(0, index).addClass('hover');

        $container.find('.bg-fill').css({ width: 100 / $btns.length * index + '%' });
    },
    hoverOutHandler: function () {
        var $container = $(this.options.container);
        clearTimeout(this.hoverHandlerTimeout);

        this.hoverHandlerTimeout = setTimeout(function () {
            $container.find('.soge-scale-btn').removeClass('hover');
            $container.find('.bg-fill').css({ width: 0 });
        }, 600);
    },
...
function lock(handler, timeout) {
    var func = handler;
    var throttleHandler = function () {
        throttleHandler.timeout = setTimeout(function () {
            func = handler;
        }, timeout);
    ...
    throttleHandler.original = handler;

    return throttleHandler;
}

$.fn.sogeBtn = function () {
    var effectsHTML = '<div class="soge-btn-effect"><span class="effect-inner"></span></div>';

    return $(this).each(function () {
        var $this = $(this);

        $this.find('.soge-btn').each(function () {
            var $btn = $(this);
            var txt = $btn.text();
            $btn.html('<span class="btn-text">' + txt + '</span>');
            $btn.append(effectsHTML);

            var btnWidth = $btn.width();
            var duration = btnWidth / 100 * 125 + 250;
            ...
            var handler = function (event) {
                ...
                $btn.find('.effect-inner').css({
                    ...
                    left: deltaX - btnWidth + 'px',
                    top: deltaY - btnWidth + 'px',
                    opacity: 1
                }, duration, function () {
                    $btn.find('.effect-inner').animate({
                        opacity: 0
                    }, 200);
            ...
            $btn.on('click', lock(handler, duration + 200));
        });
    });
};
var QuestionReader = function (questionsJSON, mainGroup, onComplete) {

    var _current;
    var _queue;

    var _questions = questionsJSON.questions;
    var _groups = questionsJSON.questionGroups.groups;
    ...
    var init = function () {
        _queue = _groups[mainGroup].items.map(getQuestion);
    };

    var addToQueue = function (questions) {
        _queue = questions.map(getQuestion).concat(_queue);
    };
    ...
    /* Филтрира всички range-ове и взима последния */
    /* Example:
     * rangeOptions = [2, 5, 7]
     * refs = ['uuid-01', 'uuid-02', 'uuid-03'] => next question group
     * answer = 3
     * getRangeReference(rangeOptions, refs, answer) => 'uuid-02'
     * => matchRanges = [0, 1,2,3]
     * returns
     */
    var getRangeReference = function (rangeOptions, refs, answer) {
        var matchRanges = _.filter([0].concat(_current.rangeOptions.ranges), function (r) {
            return r <= answer;
        });

        return refs[matchRanges.length - 1];
    };

    /**
     * Връща групата с въпроси към който сочи съответния отговор
     * @param {[{text: String, ref: String}]} options
     * @param {int} answer - index of the answer
     */
    var getOptionRef = function (options, answer) {
        return options[answer] ? options[answer].ref : '';
    };
    ...
    this.getNextQuestion = function (answer) {
        // first time we call getNextQuestion, _current is undefined
        if (!_current) return getNextQuestion();

        var ref;

        if (_current.rangeOptions) {
            ref = getRangeReference(_current.rangeOptions.ranges, _current.rangeOptions.rangeRefs, answer);
        }

        if (_current.options.length) {
            ref = getOptionRef(_current.options, answer);
        }

        if (ref) {
            addToQueue(_groups[ref].items);
        }
...
var waitForNextQuestion = false;
var sendTo = {
    url: $('.soge-young-chatbot').attr('data-url'),
    method: 'post'
};
...
var SoGeYoung = {
    ...
    init: function (questions) {
        SoGeYoung.qReader = new QuestionReader(questions, 'MAIN', SoGeYoung.onComplete);
        var mainQuestion = SoGeYoung.qReader.getNextQuestion();
        SoGeYoung.createViewFactory(mainQuestion);
    },
    createViewFactory: function (question) {
        $('.soge-expressions').removeClass('show-smile').removeClass('show-error');

        SoGeYoung.animateQuestionText(question.text, SoGeYoung.showAnswerControls);
    },
    showAnswerControls: function () {
        var question = SoGeYoung.qReader.getCurrent();
        // console.log(question);
        $('.soge-expressions').addClass('show-smile');
        if (isCitySelect(question)) {
            renderCitiesSelect(question);
        } else if (isRangeQuestion(question)) {
            renderScale(question);
        } else if (isOptionsQuestion(question)) {
            renderOptions(question.options);
        } else {
            renderTextInput(question.options);
        }
        if (isLastQuestion(question)) {
            renderLastQuestion(question);
        }
        /* if (isLangQuestion(question)) {
                renderLangQuestion(question.options);
        } */
        // SoGeYoung.showSmile();
        $('.soge-answer').animate({
            opacity: 1
    ...
    showError: function (repeat) {
    },
    animateQuestionText: function (text, done) {
        if (SoGeYoung.typed != null) {
            SoGeYoung.typed.destroy();
        }
        ...
        if (SoGeYoung.qReader.isComplete() === false) {
            SoGeYoung.showLoading();
        }
        SoGeYoung.typed = new Typed('.soge-question>div', typedOptions);
    },
    onAnswer: function (answer) {
        if (waitForNextQuestion) return;

        var current = SoGeYoung.qReader.getCurrent();
        var next = SoGeYoung.qReader.getNextQuestion(answer);

        if (next !== undefined) {
            waitForNextQuestion = true;
            SoGeYoung.collectAnswers(current, answer);

            setTimeout(function () {
                SoGeYoung.fadeOutControls(function () {
                    SoGeYoung.createViewFactory(next);
                    waitForNextQuestion = false;
                });
            }, 1000);
        }
    },
    collectAnswers: function (question, answer) {
        var ans = '';

        if (!isOptionsQuestion(question)) ans = answer; else ans = question.options[answer].text;

        SoGeYoung.chatbot.answers.push({
            question: question.text,
            answer: ans
        });
    },
    fadeOutControls: function (done) {
        $('.soge-answer').animate({
            opacity: 0
        }, done);
    },
    onComplete: function () {
        var formInfo = $('#get-results-email.chatbot').serialize();
        var chatBotData;
        SoGeYoung.chatbot.form.push(formInfo);
        if (_loggedUser.length == 0) {
            chatBotData = {
                res: JSON.stringify(SoGeYoung.chatbot.answers),
                form: formInfo
            };
        } else {
            chatBotData = {
        ...
        $.ajax($(Object).extend({
            ...
            success: function (data) {
                ...
                try {
                    if (parsed.status === false && checkAnswers > 0) {
                        if ($('form').hasClass('submitted-form')) {
                            $('form').removeClass('submitted-form');
                        }
                        $.each(parsed.errors, function (key, val) {
                            var $error_label = $("#" + key).parent();
                            var $tooltips = $("#" + key).parent().find('.tooltip-div');
                            $tooltips.text(val);
                            $error_label.addClass('error-fld');
                        });
                    } else if (checkAnswers == 0) {
                        $('.soge-answer').empty();
                        return SoGeYoung.animateQuestionText("No problem!");
                    } else {
                        $('.soge-answer').empty();
                        SoGeYoung.success();
                        $(".soge-btn, .soge-answer").unbind('click');
                        $(".soge-answer").animate({
                            opacity: 0
                        }, 400, function () {
                            $(".soge-answer").addClass('disable');
                        });
                    }
                } catch (e) {
                    //HTML
            ...
            .catch(function () {
                SoGeYoung.fail();
                SoGeYoung.showError(false);
            });
    },

    success: function () {
        SoGeYoung.animateQuestionText("Your answers were processed!");
    },
    fail: function () {
        SoGeYoung.animateQuestionText("An error has occurred!");
    }
};

function isRangeQuestion(q) {
    return q.rangeOptions;
}

function isOptionsQuestion(q) {
    return q.options.length;
}

function isCitySelect(q) {
    return q.topic === "city";
}
function isLastQuestion(q) {
    return q.topic === "finish";
}

function renderOptions(options) {
    var $container = $(".soge-answer");
    $container.html("<div class='soge-btn-wrapper'></div>");
    $container = $container.children();
    var containerClassName;

    switch (options.length) {
        case 1:
            containerClassName = 'btn-1';
            break;
        case 2:
            containerClassName = 'btn-2';
            break;
        case 3:
            containerClassName = 'btn-3';
            break;
        default:
            containerClassName = 'btn-n';
    }

    $container.addClass(containerClassName);

    var $btns = _.map(options, function (opt, index) {
        var $btn = $("<span class='soge-btn'></span>");
        $btn.text(opt.text);
        $btn.on('click', function () {
            SoGeYoung.onAnswer(index);
        });
...
function renderScale() {
    var $container = $('.soge-answer').empty();
    $container.html("<div class=\"soge-scale\">\n        <div class=\"soge-scale-btn\">\n            <div class=\"soge-scale-btn-label\">1</div>\n        </div>\n        <div class=\"soge-scale-btn\">\n            <div class=\"soge-scale-btn-label\">2</div>\n        </div>\n        <div class=\"soge-scale-btn\">\n            <div class=\"soge-scale-btn-label\">3</div>\n        </div>\n        <div class=\"soge-scale-btn\">\n            <div class=\"soge-scale-btn-label\">4</div>\n        </div>\n        <div class=\"soge-scale-btn\">\n            <div class=\"soge-scale-btn-label\">5</div>\n        </div>\n        <div class=\"bg-fill\"></div>\n    </div>");

    new ScaleView({
        container: $container,
        onClick: SoGeYoung.onAnswer
    });
}
...
function renderTextInput() {
    ...
    $container.html("\n        <div class=\"soge-input-wrapper\">\n            <div class=\"soge-input\">\n                <input type=\"text\" class=\"empty\" id=\"soge-input\" />\n                <label for=\"soge-input\">Please your input here</label>\n                <div class=\"soge-input-line\"></div>\n            </div>\n            <span class=\"soge-btn\">Send</span>\n        </div>\n    ");
    ...
    var next = lock(function () {
        var $input = $container.find('input');
        var val = $input.val();

        if (_.trim(val)) {
            SoGeYoung.onAnswer(val);
    ...
    $container.sogeBtn();
    $container.on('click', '.soge-btn', next);
...
function shake($container, callback) {
    $container.addClass('soge-shake').on('animationend', function () {
        $container.removeClass('soge-shake');
...
function renderCitiesSelect() {
    ...
    var $select = $("\n    <div class=\"soge-select-wrapper\">\n        <div class=\"soge-select\">\n            <label class=\"label\" for=\"city-select\">\n                <span class=\"label-text\">\u0418\u0437\u0431\u0435\u0440\u0438 \u0433\u0440\u0430\u0434</span>\n                <div class=\"caret\"></div>\n            </label>\n            <div class=\"soge-input-line\"></div>\n            <div class=\"soge-list-wrapper reverse\"></div>\n        </div>\n        <div class=\"soge-btn btn-small soge-next\">\u0418\u0437\u0431\u0435\u0440\u0438</div>\n    </div>");
    $container.append($select);

    if ($(window).width() <= 1024) {
        $dropdown = getMobileDropdown();
        $select.find('.label').append($dropdown);
    } else {
        $dropdown = getDesktopDropdown();
        $select.on('click', '.label', function () {
            $select.find('.soge-list-wrapper').toggleClass('open');

            // open the dropdown in reverse
            // console.log($select.offset().top + 200 , $(window).height() - 50);
            if ($select.offset().top + 200 > $(window).height() - 50) {
                $select.find('.soge-list-wrapper').addClass('reverse');
            } else {
                $select.find('.soge-list-wrapper').removeClass('reverse');
            }
        }).addClass('align-arrows');
        $select.find('.soge-list-wrapper').append($dropdown);
    }

    $select.on('change', function (event) {
        selectedValue = event.target.value;
        $select.find('.label-text').text(event.target.value);
        $select.find('.soge-list-wrapper').removeClass('open');
        renderMapsLink(event.target.value);
    });

    $select.on('click', '.soge-next', function () {
        if (_.trim(selectedValue)) {
            SoGeYoung.onAnswer(selectedValue);
        } else {
            shake($('.soge-select'));
        }
    });
...
function renderMapsLink(city) {
    city = city == 'друг' ? 'България' : city;
    var $btn = $('<a class="soge-btn no-m p15" target="_blank">Покажи на картата</a>');
    // $btn.attr('href', "https://www.google.bg/maps/search/"+city+"+експресбанк' OR '"+city+"ДСК'");
    $btn.attr('href', "https://www.google.bg/maps/search/\"\u0435\u043A\u0441\u043F\u0440\u0435\u0441\u0431\u0430\u043D\u043A\"+OR+\"\u0411\u0430\u043D\u043A\u0430 \u0414\u0421\u041A\"+NEAR+\"" + String(city) + "\"");

    var $container = $('.soge-select-wrapper');
    $container.find('a.soge-btn').remove();
    $container.append($btn);
}
function renderLastQuestion() {
    var $container = $('.soge-answer');
    var gdprUrl = _root_full + 'terms-and-conditions';
    if (_loggedUser.length == 0) {
        // check if user is logged in  ${_root}assets/js/chatbot.json
        $container.html("\n        <div class=\"wrapper login-section chatbot\">\n            <form action=\"" + String($('.soge-young-chatbot').attr('data-url')) + "\" method=\"POST\" id=\"get-results-email\" class=\"chatbot\">\n                <div class=\"form-row required\">\n                    <label for=\"name\" class=\"label-form-fld\">Name</label>\n                    <input type=\"text\" class=\"form-fld\" id=\"name\" name=\"name\">\n                    <div class=\"border-bottom\"></div>\n                    <div class=\"tooltip-div\"></div>\n                </div>\n                    <div class=\"form-row required\">\n                    <label for=\"email\" class=\"label-form-fld\">E-mail</label>\n                    <input type=\"text\" class=\"form-fld\" id=\"email\" name=\"email\">\n                    <div class=\"border-bottom\"></div>\n                    <div class=\"tooltip-div\"></div>\n                </div>\n                <div class=\"form-row row-checkbox\">\n                    <input type=\"checkbox\" value=\"1\" class=\"form-fld\" id=\"hr-checkbox\" name=\"hr-checkbox\">\n                    <label for=\"hr-checkbox\" class=\"label-form-fld\">I agree with <a href=\"" + gdprUrl + "\" class=\"conditions-link\" target=\"_blank\">General terms and Conditions</a></label>\n                    <div class=\"tooltip-div\"></div>\n                </div>\n                <div class=\"form-row btn-row chatbot\">\n                    <button class=\"submit-btn\" type=\"submit\" id=\"submit-chatbot\">Send</button>\n                    <button class=\"submit-btn\" type=\"submit\" id=\"dont-submit-chatbot\">No</button>\n                </div>\n            </form>\n            <div class=\"success-msg\">\n                <span class=\"success-text\"></span>\n            </div>\n        </div>\n        ");
    } else {
        $container.html("\n        <div class=\"wrapper login-section chatbot\">\n            <form action=\"" + String($('.soge-young-chatbot').attr('data-url')) + "\" method=\"POST\" id=\"get-results-email\" class=\"chatbot\">\n                <div class=\"form-row btn-row chatbot\">\n                    <button class=\"submit-btn\" type=\"submit\" id=\"submit-chatbot\">\u0418\u0437\u043F\u0440\u0430\u0442\u0438</button>\n                    <button class=\"submit-btn\" type=\"submit\" id=\"dont-submit-chatbot\">\u041D\u0435</button>\n                </div>\n            </form>\n        </div>\n        ");
    }
    $('#dont-submit-chatbot').on('click', function (e) {
        e.preventDefault();
        SoGeYoung.animateQuestionText("No problem, see ya. ;)");
        $('.soge-answer').empty();
    });
    $('#get-results-email.chatbot').on('submit', function (e) {
        e.preventDefault();
        $(this).addClass('submitted-form');
        SoGeYoung.onComplete();
    });
}